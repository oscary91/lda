...
namespace flexps {
	...
	void init_tables(std::vector<flexps::LDADoc>& corpus, Info info, int LDATableId, int num_topics, int max_voc_id) {
	...
	//void batch_training_by_chunk(std::vector<flexps::LDADoc>& corpus, int LDATableId, int num_total_workers, int num_topics, int max_voc_id, int num_iterations,	
	void batch_training_by_chunk(std::vector<flexps::LDADoc>& corpus, KVClientTable<float>& lda_table, KVClientTable<float>& lda_stat_table, KVClientTable<float>& llh_progress_table, int num_total_workers, int num_topics, int max_voc_id, int num_iterations,	
		...
		std::vector<flexps::Key> stat_keys(9);
		...
		for (int i=1; i<=num_iterations; i++) {
			...
			for (int j = 0; j<num_batches; j++) {
				...
				while (iter != batch_voc_set.end()) {
					...
					//for (int count = 0; iter != batch_voc_set.end(); count ++) {
...
//						LOG(INFO) << " gen 2d key time: " << key_2d_gen_duration.count();
					...
					find_start_word_topic_idx[max_voc_id] = batch_voc_topic_keys.size() - num_topics;
					...
					for (int k = batch_begin[j]; k < batch_end[j]; k++) {
						...
						//LOG(INFO) << info.DebugString() << "Debug: Before - " << corpus[k].get_num_tokens() << " word_topic_count.size(): " << word_topic_count.size() << " word_topic_updates.size(): " << word_topic_updates.size() << " find_start_word_topic_idx.size(): " << find_start_word_topic_idx.size() << " batch_voc_topic_keys.size(): " << batch_voc_topic_keys.size() << " batch_voc_keys.size(): " << batch_voc_keys.size();
						
						doc_sampler.sample_one_doc(corpus[k], word_topic_count, word_topic_updates, find_start_word_topic_idx);
						//LOG(INFO) << info.DebugString() << "Debug: After - " << corpus[k].get_num_tokens() << " word_topic_count.size(): " << word_topic_count.size() << " word_topic_updates.size(): " << word_topic_updates.size() << " find_start_word_topic_idx.size(): " << find_start_word_topic_idx.size() << " batch_voc_topic_keys.size(): " << batch_voc_topic_keys.size() << " batch_voc_keys.size(): " << batch_voc_keys.size();
					}
						...
						LOG(INFO) << " sampling time: " << sample_time.count();
					{
						...
						//LOG(INFO) << info.DebugString() << " Before Add(): " << "batch_voc_topic_keys.size()" << batch_voc_topic_keys.size() << " word_topic_updates.size: " << word_topic_updates.size(); 
			...
			if (need_compute_llh) {
				...
				int topic_summary_start = find_start_word_topic_idx[max_voc_id];
				int sum = 0;
...
//###
			std::vector<Key> llh_progress_keys;
			std::vector<float> progress_count_params;
			int num_llh_progress_table_column = num_total_workers + 1;
			llh_progress_keys.push_back((i-1) * num_llh_progress_table_column); // iteration count from 1
			LOG(INFO) << info.DebugString() << " llh_progress_keys.size(): " <<llh_progress_keys.size();
			llh_progress_table.Get(llh_progress_keys, &progress_count_params);
			progress_count_params[0] = 1;	
			llh_progress_table.Add(llh_progress_keys, progress_count_params);
			llh_progress_table.Clock();	
			if ( info.worker_id == 0 )
			{
				llh_progress_keys.clear();
				progress_count_params.clear();
				for (int ii = 0; ii < num_iterations; ii++)
					llh_progress_keys.push_back(ii * num_llh_progress_table_column);
				LOG(INFO) << info.DebugString() << " Only llh_progress_keys.size(): " <<llh_progress_keys.size();
				llh_progress_table.Get(llh_progress_keys, &progress_count_params);
				std::ofstream of;
				std::ios_base::iostate exceptionMask = of.exceptions() | std::ios::failbit;	
				of.exceptions(exceptionMask);
				of.open("/data/opt/tmp/1155004171/output/counter.txt", std::ofstream::out | std::ofstream::app);
				of << "===========================================\n";
				of << "progress_count: " << std::to_string(progress_count) << "\n"; 
				for (int ii = 0; ii < num_iterations; ii++)
				{
					std::string writeline = std::to_string(ii) + ": " + boost::lexical_cast<std::string>(progress_count_params[ii]) + "\n";
					of << writeline;
				}
				if (progress_count_params[0] == num_total_workers)
				{
					progress_count++;		
				}
			}
		...
		void Run() {
			...
			const int lda_stat_table_dim = FLAGS_num_iters * 9;
			for (int i = 0; i < num_total_servers - 1; ++ i) {
			...
			// 3.2 Create LLH and progress monitor table
			const int LLHProgressTableId = 2;
			std::vector<third_party::Range> llh_progress_table_range;
			const int llh_progress_table_dim = FLAGS_num_iters * (num_total_workers + 1);
			for (int i = 0; i < num_total_servers - 1; i++)
			{	
				llh_progress_table_range.push_back({(llh_progress_table_dim / num_total_servers * i), (llh_progress_table_dim / num_total_servers * (i + 1))});
			}
			llh_progress_table_range.push_back({(llh_progress_table_dim / num_total_servers * (num_total_servers - 1)), llh_progress_table_dim});
			engine.CreateTable<float>(LLHProgressTableId, llh_progress_table_range, ModelType::ASP, StorageType::Vector, 0, 0, SparseSSPRecorderType::None);
			...
			task.SetTables({LDATableId, LDAStatTableId, LLHProgressTableId});  // Use word_topic_table: 0
			task.SetLambda(
					[LDATableId, LDAStatTableId, LLHProgressTableId, &corpus, num_topics, max_voc_id, num_total_workers]
					(const Info& info)
					{
					...
					auto llh_progress_table = info.CreateKVClientTable<float>(LLHProgressTableId);
					batch_training_by_chunk(local_corpus_by_worker, lda_table, lda_stat_table, llh_progress_table, num_total_workers, num_topics, max_voc_id, num_iterations, alpha, beta, info);
			...
			task_output.SetWorkerAlloc(worker_alloc);
			...
			task_output.SetLambda(
					...
					std::stringstream timestamp_now;
					//std::string outfile_name = "/data/opt/tmp/1155004171/output-word-topic" + std::string(std::put_time(std::localtime(&now_c), "%F-%T")) + ".txt";
					//LOG(INFO) << outfile_name;
					if (info.worker_id == 0)
					{
						timestamp_now << std::put_time(std::localtime(&now_c), "%Y-%m-%d_%H%M%S");
						std::string output_file_name = "/data/opt/tmp/1155004171/output/output-word-topic-" + timestamp_now.str() +".txt";
						of.open(output_file_name, std::ofstream::out | std::ofstream::app);

						// write num_topics								
						std::string writeline = boost::lexical_cast<std::string>(FLAGS_num_topics) + "\n";
						of << writeline;
						// write max_voc_id
						writeline = boost::lexical_cast<std::string>(FLAGS_max_voc_id) + "\n";
						of << writeline;
